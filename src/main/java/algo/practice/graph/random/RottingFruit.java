 ...
 * - To store this rotting time for each fruit,
 *      - we need either a minRottingTimeMatrix[][].
 *      - Else we can also store this value in Queue<int[]{i, j, rottingTime}>.
...
public class RottingFruit {
    ...
    public int orangesRottingV2(int[][] grid) {
        ...
        while(!queue.isEmpty()){
            ...
            int r = cell[0];
            int c = cell[1];
            int minRottingTimeForQueueElem = cell[2];
            ...
            for(int[] dir : directions){
                ...
                freshBananaCount--;
                queue.add(new int[]{nextRow, nextColumn, minRottingTimeForQueueElem+1});
                //update minRottingTime
                minRottingTime = Integer.max(minRottingTime, minRottingTimeForQueueElem+1);
            }
        }
