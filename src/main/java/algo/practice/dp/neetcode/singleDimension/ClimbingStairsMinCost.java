package algo.practice.dp.neetcode.singleDimension;

/**
 * You are given an array of integers cost where cost[i] is the cost of taking a step from the ith floor of a staircase.
 * After paying the cost, you can step to either the (i + 1)th floor or the (i + 2)th floor.
 *
 * You may choose to start at the index 0 or the index 1 floor.
 *
 * Return the minimum cost to reach the top of the staircase, i.e. just past the last index in cost.
 *
 * Example 1:
 *
 * Input: cost = [1,2,3]
 *
 * Output: 2
 * Explanation: We can start at index = 1 and pay the cost of cost[1] = 2 and take two steps to reach the top. The total cost is 2.
 *
 * Example 2:
 *
 * Input: cost = [1,2,1,2,1,1,1]
 *
 * Output: 4
 *
 *
 *
 * dp[n] = Min(dp[n-1]+ cost[n-1], dp[n-2]+cost[n-2])
 *
 */
public class ClimbingStairsMinCost {
    public int minCostClimbingStairs(int[] cost) {
        int n = cost.length;
        int[] dp = new int[n+1];
        return findMinCostForClimbingStairs(cost, dp, n);
    }

    private int findMinCostForClimbingStairs(int[] cost, int[] dp, int stairNum) {
        if(stairNum==0){
            return 0;
        }
        if(stairNum==1){
            return cost[0];
        }
        if(stairNum==2){
            return Math.min(cost[0], cost[1]);
        }
        if(dp[stairNum] == 0){
            dp[stairNum] = Math.min(
                    cost[stairNum-1] + findMinCostForClimbingStairs(cost, dp, stairNum-1),
                    cost[stairNum-2] + findMinCostForClimbingStairs(cost, dp, stairNum-2)
            );
        }
        return dp[stairNum];
    }
}
